...
There are a few vways to do so:
...
If we abort the request during the fetch, the browser will be notified and do something about it. But if the abortion happens while the browser is running the `then()` callback, it has no way to handle the abortion of this part of the code, and you have to write this logic on your own. If the abortion happens during the fake delay we added, it will not cancel that delay and stop the flow.

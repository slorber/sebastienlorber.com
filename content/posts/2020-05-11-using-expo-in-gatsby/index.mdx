...
- **React-Native-Web**: it is a CSS-in-JS library (similar to Emotion), using the React-Native APIs, with interesting properties (like [atomic CSS-in-JS](https://sebastienlorber.com/atomic-css-in-js)).
...
>
  <TouchableOpacity
    onPress={() => alert("You though it wouldn't work?")}
    style={{
      padding: 10,
      backgroundColor: 'blue',
      borderRadius: 5,
...
I have good hope to see more React-Native developers embed **runnable code** in their blogs, like I do in this post!
...
After adding the required dependencies, you only need to add the plugin to `gatsby-config.js`, and **nothing else is required** (0 plugin config to provide). This blog post needs 0 config to run properly.
...
Check the React-Native-Web and Expo docs for available web platform support, as not all components have web support.
...
Keep in mind that all the demos are written with React-Native and Expo components / apis.
There's no usage of a single `div`, or any direct browser API usage.
...
We need more people to adopt cross-platform development, to make it more mainstream and polished.

As the setup is easier than ever, it's a good time to jump on the bandwagon, and get started with React-Native-Web.

...
Many blog articles talk about loading data in React apps, weither it's ad-hoc using `componentDidMount/update` or `useEffect`, or through Redux, Apollo...

Yet, all those articles are **generally optimistic**, and never mention something important to consider: concurrency issues could happen, and you may end up rendering the wrong data.
...
If there is a non-null probability that your UI end up in such a state, then your app is subject to concurrency issues.

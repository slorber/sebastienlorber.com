...
In this post, we'll see what atomic CSS is, how it relates to utility-first CSS like Tailwind, and that big players are adopting it in their modern React codebases.
...
- CSS rules insertion order matters
...
I'm not aware about all CSS-in-JS libs currently supporting atomic CSS. Supporting atomic CSS is an implementation detail of the CSS-in-JS lib. One lib might not support it now, but might start supporting it in the future, without changing its API.

In this post, I want to highlight 2 particular solutions, that recently lead to 2 large-scale atomic CSS deployments:
...
I'll show screenshots of these talks that are related to atomic CSS.
...
React-Native-Web is a very interesting library: it permits to render React-Native primitives on the web. But but we are not really talking about cross-platform mobile/web development here, watch the talks for more details.

As a web developer, you just need to understand that React-Native-Web is a regular CSS-in-JS library, that comes with a few very basic React component primitives.
...
Unfortunately, all the infos I have are from the talk :) We'll have to wait for more details.
...
As expected with atomic CSS, we see that both Twitter and Facebook a massive reduction of their CSS, as it now follows a logarithmic curve as the number of component grows.
...
The 2 libraries looks quite similar here, but it's hard to say, considering we don't know much about Stylex.
...
As you may know, in case of conflicting rules, it's not the last class of an attribute that wins, but the rule that is last in the stylesheet. I don't mention specificity rules, because here all selectors are simple class-based ones.

![](./images/rnw-rules-order.png)

![](./images/stylex-rules-order.png)

In practice, these libraries avoid outputting classes with conflicting rules on the same element, and ensure that the last style declared in the markup always wins.
...
<!-- That div will have a single atomic class for the blue color -->
<div style={[styles.red, styles.blue]}>
  Always blue!
</div>

<!-- That div will have a single atomic class for the red color -->
...
I'm not really an expert of this subject (never used atomic CSS, CSS-in-JS, or Tailwind in production), so I'm probably wrong on some parts. The goal of this article is not to be a deep dive but rather to expose you the idea.
...
As I've not been able to find any article about atomic CSS-in-JS, I wrote this mostly for myself, to be able to link to something in upcoming blog posts, as I plan to write more articles about React-Native-Web.
